  ...
  var filterProducts = function () {
    ...
    function updateMetaFilter() {
      ...
      if (filters.availability) {
        appliedFilters.append(
          `<span className="filter-tag">${filters.availability} <span className="remove-tag icon-close" data-filter="availability"></span></span>`
        );
      }
      if (filters.size) {
        appliedFilters.append(
          `<span className="filter-tag">${filters.size} <span className="remove-tag icon-close" data-filter="size"></span></span>`
        );
      }
      if (filters.minPrice > minPrice || filters.maxPrice < maxPrice) {
        appliedFilters.append(
          `<span className="filter-tag">$${filters.minPrice} - $${filters.maxPrice} <span className="remove-tag icon-close" data-filter="price"></span></span>`
        );
      }
      if (filters.color) {
        ...
        appliedFilters.append(
          `<span className="filter-tag color-tag">
                  <span className="color ${backgroundClass} ${line}"></span>
                  ${filters.color}
                  <span className="remove-tag icon-close" data-filter="color"></span>
              </span>`
        );
      }
      ...
      if (filters.brands.length > 0) {
        filters.brands.forEach((brand) => {
          appliedFilters.append(
            `<span className="filter-tag">${brand.label} <span className="remove-tag icon-close" data-filter="brand" data-value="${brand.id}"></span></span>`
          );
        });
      }
      ...
      if (filters.sale) {
        appliedFilters.append(
          `<span className="filter-tag on-sale d-none">On Sale <span className="remove-tag icon-close" data-filter="sale"></span></span>`
        );
      }
    ...
    function applyFilters() {
      ...
      $("#product-count-grid").html(
        `<span className="count">${visibleProductCountGrid}</span> Products Found`
      );
      $("#product-count-list").html(
        `<span className="count">${visibleProductCountList}</span> Products Found`
      );
